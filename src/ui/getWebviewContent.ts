...
import { Note , SqlEditor} from "../types/Note";
...
`;
  ...
  export function getWebviewContentSql(webview: Webview, extensionUri: Uri, sql: SqlEditor) {
    ...
    const nonce = getNonce();
    const formattedTags = sql.tags ? sql.tags.join(", ") : null;
    ...
    <html lang="en">
      <head>
          ...
          <title>${sql.QUERY_NAME}</title>
      </head>
      <body id="webview-body">
        <header>
          <h1>${sql.QUERY_NAME}</h1>
          <div id="tags-container"></div>
        </header>
        <section id="notes-form">
        <vscode-divider role="separator"></vscode-divider>
        
        <vscode-text-field id="title" value=${sql.QUERY_NAME} placeholder="Enter a name" type="text">Sql Map Query*</vscode-text-field>
        ...
        <vscode-dropdown id="type-dropdown" position="below">
          ...
          <vscode-text-area id="editor"value="${sql.SQL_0}" placeholder="Query 작성!" resize="vertical" rows=15></vscode-text-area>
          <div id="editor" class="monaco-editor-container"></div>
          <vscode-divider role="separator"></vscode-divider>
          ...
          <vscode-divider role="separator"></vscode-divider>
          <vscode-text-area id="description"value=${sql.DESCRIPTION} placeholder="" resize="vertical" rows=5>Description</vscode-text-area>
          <div>
            <vscode-button id="submit-button3">Query Save</vscode-button>
          </div>
        </section>
