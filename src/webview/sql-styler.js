...
module.exports = function format(text) {
    ...
    while (tokens.length) {
        ...
        if (['/*','*/'].includes(keyword) || stack.peek() === 'COMMENT') {
            ...
            // Do not update last_keyword in-comment state.
        ...
        if (['SELECT', 'FROM', 'WHERE', 'AND', 'GROUP BY', 'ORDER BY',
            ...
            'HAVING', 'BETWEEN', 'CREATE', 'TABLE', 'HAVING',
            'INSERT', 'INTO', 'OVERWRITE', 'VALUES'].includes(keyword)) {
            ...
            } else if (['INTO', 'OVERWRITE'].includes(keyword)) {
                formatted.push(' ');
